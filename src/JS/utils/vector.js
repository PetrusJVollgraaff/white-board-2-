class Vector {
  ...
  static formOffsets(info) {
    return new Vector({ x: info.offsetX, y: info.offsetY });
  }
  ...
  static topLeft(vectors) {
    ...
    for (const vector of vectors) topLeft = topLeft.min(vector);
  ...
  scale(scaler) {
    return new Vector({ x: this.x * scaler, y: this.y * scaler });
  }

  min(v) {
    return new Vector({ x: Math.min(this.x, v.x), y: Math.min(this.y, v.y) });
  }

  max(v) {
    return new Vector({ x: Math.max(this.x, v.x), y: Math.max(this.y, v.y) });
  }
